...
final class DefaultRSocketRequester implements RSocketRequester {
	...
	private class DefaultRequestSpec implements RequestSpec {
		...
		public RequestSpec data(Object data) {
			data(data, EMPTY_HINTS);
			return this;
		}

		@Override
		public RetrieveSpec data(Object data, Map<String, Object> hints) {
			Assert.notNull(data, "'data' must not be null");
			createPayload(data, ResolvableType.NONE, hints == null ? EMPTY_HINTS : hints);
			return this;
		}
		...
		public RequestSpec data(Object producer, Class<?> elementClass) {
			...
			createPayload(adapter.toPublisher(producer), ResolvableType.forClass(elementClass), EMPTY_HINTS);
			return this;
		}
		...
		public RequestSpec data(Object producer, ParameterizedTypeReference<?> elementTypeRef) {
			...
			ReactiveAdapter adapter = getAdapter(producer.getClass());
			Assert.notNull(adapter, "'producer' type is unknown to ReactiveAdapterRegistry");
			createPayload(adapter.toPublisher(producer), ResolvableType.forType(elementTypeRef), EMPTY_HINTS);
			return this;
		}

		private void createPayload(Object input, ResolvableType elementType, Map<String,Object> hints) {
			...
			if (input instanceof Publisher) {
				...
				this.payloadMono = firstPayload(Mono.fromCallable(() -> encodeData(input, type, null, hints)));
			...
			if (adapter != null && !adapter.isMultiValue()) {
				Mono<DataBuffer> data = Mono.from(publisher)
						.map(value -> encodeData(value, elementType, encoder, hints))
		...
		private <T> DataBuffer encodeData(T value, ResolvableType elementType, @Nullable Encoder<?> encoder,Map<String,Object> hints ) {
			if (encoder == null) {
